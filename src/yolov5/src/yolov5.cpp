...
#define NMS_THRESH 0.35   // 非极大值抑制范围（一个物体被识别了多个，选取置信度高的）
#define CONF_THRESH 0.75  //置信度
#define BATCH_SIZE 1
...
static float data[BATCH_SIZE * 3 * INPUT_H * INPUT_W]; // 输入图片
static float prob[BATCH_SIZE * OUTPUT_SIZE];           // 输出图片
...
{
public:
    yolov5_ros2(std::string name) : Node(name)
    {
        ...
        img_subscribe = this->create_subscription<sensor_msgs::msg::Image>(img_sub_topic.c_str(), 10, std::bind(&yolov5_ros2::img_callback, this, std::placeholders::_1));
...
private:
    void img_callback(const sensor_msgs::msg::Image::SharedPtr msg)
    {
        ...
        cv::Mat pr_img = preprocess_img(BGR, INPUT_W, INPUT_H);
        ...
        boxes.header = cv_color_ptr->header;
        for (size_t j = 0; j < res.size(); j++)
        {
            ...
            cv::putText(BGR, jetson_fps, cv::Point(11, 80), cv::FONT_HERSHEY_PLAIN, 3, cv::Scalar(0, 0, 255), 2, cv::LINE_AA);
    ...
    std::string img_sub_topic = "";
    std::string img_pub_topic = "";
    std::string box_pub_topic = "";
    // Create stream
    cudaStream_t stream;
...
int main(int argc, char **argv)
{
    ...
    if (!parse_args(argc, argv, wts_name, engine_name, is_p6, gd, gw, img_dir))
    {
        ...
        rclcpp::spin(std::make_shared<yolov5_ros2>("yolov5_ros2"));
        rclcpp::shutdown();
        return 0;
    }
